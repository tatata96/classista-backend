...
2. Checkpoint 3: configure the Supabase project and NestJS environment variables (`SUPABASE_URL`, the token verification credential, any CORS setting the frontend origin needs), and set up a free tier SMTP provider so Supabase's transactional email reaches real users, satisfies **AC-7**, **AC-8**.
3. Checkpoint 4: build the Supabase token verification service (isolated in its own provider) and the NestJS auth guard that rejects missing, invalid, or expired tokens, satisfies **AC-1**, **AC-2**, **AC-3**.
4. Checkpoint 5: implement `GET /auth/me`, resolving the verified token to a Classista `User` (creating the row on first sight if that is the mapping timing chosen at Checkpoint 2) and returning a safe profile shape, satisfies **AC-4**, **AC-5**, **AC-6**.

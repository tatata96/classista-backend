...
5. ~~Checkpoint 6: write tests for AC-1 through AC-9, and update Swagger and the README~~ — done: 38 automated tests across the four `src/auth/` files (unit tests mocking `jose` and Prisma at the boundary), the missing Swagger bearer scheme registration in `main.ts` fixed so `@ApiBearerAuth()` actually resolves, and the README updated with the new endpoint and `SUPABASE_URL`. AC-8 (SMTP) stays untested by design, it is dashboard configuration with no code to test. The real-token happy path is still unexercised against a live login; everything here mocks the verification/DB boundary, which is correct for unit tests but is not the same as a live end to end check.
...
- [x] Checkpoint 3/4: token verification mechanism decided and built — the project's JWT Signing Keys are asymmetric (`ECC P-256`), so verification uses `jose`'s `createRemoteJWKSet` against the project's public JWKS, no shared secret involved.
- [ ] Pick a free tier SMTP provider for Supabase Auth's transactional email (confirmation, password reset) and configure it in the Supabase dashboard; Supabase's built in sender is testing only and not meant for real users. Blocks real user onboarding, not the rest of this build.
- [ ] Exercise the real, valid token happy path against a live Supabase login; every automated test so far mocks `jose`/Prisma at the boundary (correct for unit tests), so nobody has confirmed end to end that a genuine access token resolves to the right user through the running app.
...
- [ ] Several Prisma community skills are installed in this repo (`.agents/skills/prisma-client-api/`, `.agents/skills/prisma-cli/`, and others) but are not yet referenced in `CLAUDE.md`.
